...
class _IsiDataPageState extends State<IsiDataPage> {
  ...
  // Fungsi untuk mendapatkan atau membuat ID pelanggan
  Future<int> _getOrCreatePelangganId(
      String nama, String telepon, String email) async {
    final prefs = await SharedPreferences.getInstance();
    final existingId = prefs.getInt('id_pelanggan');
    if (existingId != null)
      return existingId; // Jika sudah ada, gunakan yang sudah ada
    ...
    // Cek apakah pelanggan sudah ada berdasarkan device_id
    ...
    if (checkResponse.statusCode == 200) {
      ...
      // Menggunakan operator ?? '' untuk memastikan nilai tidak null
    ...
    if (createResponse.statusCode == 200 || createResponse.statusCode == 201) {
      ...
      throw Exception('Gagal membuat pelanggan baru: ${createResponse.body}');
    }
  }
  ...
  Future<void> _savePelangganData() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        await _getOrCreatePelangganId(nama, telepon, email);
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text('Gagal menyimpan data pelanggan: $e'),
