                    ...
                    <div class="card-body">
                        <form action="{{ route('pesanan.index') }}" method="GET" class="row g-3">
                            ...
                            <div class="col-md-3 d-flex align-items-end">
                                ...
                                <a href="{{ route('pesanan.index', ['active_tab' => $activeTab]) }}"
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    @forelse($pesanan as $p)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    <span
                                                        class="badge text-bg-light">{{ ucfirst($p->metode_pembayaran) }}</span>
                                                @endif
                                            </td>
                                            <td>
                                                    ...
                                                    <ul class="dropdown-menu"
                                                        ...
                                                        <li>
                                                            <a class="dropdown-item detail-btn" href="#"
                                                                ...
                                                                data-id="{{ $p->id }}"
                                                                data-nama-pelanggan="{{ $p->pelanggan->nama ?? 'N/A' }}"
                                                        ...
                                                        @php
                                                            ...
                                                            $statusText = match ($p->status) {
                                                                ...
                                                                'diproses'
                                                                    => 'Pesanan Anda sedang kami proses di dapur.',
                                                                'selesai'
                                                                    => 'Pesanan Anda telah selesai dan siap diambil/diantar. Terima kasih!',
                                                                'dibatalkan' => 'Pesanan Anda telah dibatalkan.',
                                                                default => 'Status pesanan Anda: ' .
                                                                    ucfirst($p->status),
                                                            };
                                                        ...
                                                        @if (!in_array($p->status, ['selesai', 'dibatalkan']))
                                                            ...
                                                            @if ($p->status !== 'diproses')
                                                                <li>
                                                                    <form
                                                                        action="{{ route('pesanan.status', ['pesanan' => $p, 'status' => 'diproses']) }}"
                                                                        method="POST" class="form-status-change">
                                                                        @csrf
                                                                        <button class="dropdown-item" type="submit"
                                                                            data-status-baru="Diproses">
                                                                            <i class="fas fa-cogs me-2"></i> Tandai
                                                                            Diproses
                                                                        </button>
                                                                    </form>
                                                                </li>
                                                            ...
                                                            @if ($p->status !== 'selesai')
                                                                <li>
                                                                    <form
                                                                        action="{{ route('pesanan.status', ['pesanan' => $p, 'status' => 'selesai']) }}"
                                                                        method="POST" class="form-status-change">
                                                                        @csrf
                                                                        <button class="dropdown-item" type="submit"
                                                                            data-status-baru="Selesai">
                                                                            <i
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="7" class="text-center">Tidak ada data pesanan untuk status
                                                "{{ ucfirst($activeTab) }}"</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (detailModalElement) {
                detailModalElement.addEventListener('show.bs.modal', function(event) {
                    ...
                    document.getElementById('pesananIdModal').textContent =
                        pesananId;
                    detailPesananContent.innerHTML =
                        '<div class="text-center">Memuat data... <i class="fas fa-spinner fa-spin"></i></div>';

                    fetch(`/pesanan/${pesananId}`)
                        .then(response => {
                            if (!response.ok) {
                        ...
                        .catch(error => {
                            ...
                            detailPesananContent.innerHTML =
                                '<p class="text-danger">Gagal memuat detail pesanan. Silakan coba lagi.</p>';
                        });
                });
            }
            ...
            statusChangeForms.forEach(form => {
                form.addEventListener('submit', function(event) {
                    ...
                    const statusBaru = statusButton.dataset.statusBaru ||
                        "status baru";
